...
import { setCategory, setSubcategory, clearFilters } from "../features/filters/filterSlice";
...
const Home = () => {
  ...
  const { items: subcategories, loading: subcategoryLoading } = useSelector((state) => state.subcategories);
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Mobile Banner */}
      <div className="w-full relative overflow-hidden block md:hidden">
          ...
          <ScrollContainer>
            {categories.map((cat) => (
              ...
              >
                ...
                <p className="mt-3 text-[#001B3D] font-semibold text-center text-sm sm:text-base">
                  {cat.name}
                </p>
              </div>
            ))}
      ...
      {/* SUBCATEGORIES */}
      <section className="flex flex-col items-center mt-16 mb-20 px-4">
        <h2 className="text-3xl font-bold mb-10 text-[#001B3D]">Shop by Subcategory</h2>
      ...
      <section className="flex flex-col items-center mt-8 mb-16 px-4">
        <h2 className="text-3xl font-bold mb-10 text-[#001B3D]">Featured Products</h2>
        ...
        {productLoading ? (
          ...
          <div className="grid justify-center gap-8 max-w-7xl grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4">
